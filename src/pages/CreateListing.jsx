...
function CreateListing() {
    ...
    useEffect(() => {
        if (isMounted) {
            onAuthStateChanged(auth, (user) => {
                if (user) {
                    setFormData((preState) => { return { ...preState, userRef: user.uid } });
                }
                else {
                    navigate('/sign-in')
                }
    ...
    }, [isMounted, setFormData, auth, navigate])
    const submitHandler = async (e) => {
        e.preventDefault();
        setLoading(true);
        const imageUrls = images.split('\n').filter((e) => e.trim() !== '');
        console.log(Number(discountedPrice));
        console.log(Number(regularPrice));
        ...
        const docRef = await addDoc(collection(db, 'llistings'), formDataCopy)
        setLoading(false);
    ...
    const addImagesHandler = (e) => {
        let images = [];
        images = String(e.target.value);
        console.log(images);
        console.log(images.split('\n').filter((e) => e.trim() !== ''));
        setFormData((preState) => { return { ...preState, images: images } });
    }
    if (loading) {
        return <Spinner />
    }
        ...
        <div className="profile">
            ...
            <main>
                <form onSubmit={submitHandler}>
                    ...
                    <textarea className="formInputAddress" type="text" id="images" value={images} onChange={addImagesHandler} required></textarea>
